...
function bindModalForms(){
  $('#create-wheel-btn')?.addEventListener('click',()=>{
    ...
    const w=store.createWheel({name,slug:name.toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9-]/g,''),description:desc,category:$('#cw-cat').value,hexColor:color,coverGradient:'linear-gradient(135deg,'+color+'cc,'+color+')',isEventWheel:$('#cw-is-event').checked});
