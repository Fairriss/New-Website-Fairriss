...
function renderWheelCard(w,discover=false,creator=null){
  return '<div class="wheel-card" data-wheel-id="'+w.id+'"><div class="wheel-card-cover" style="background:'+(w.coverGradient||'var(--navy)')+'"></div><div class="wheel-card-body">'+hexBadge(w,48)+'<div class="wheel-card-name">'+escHtml(w.name)+'</div><div class="wheel-card-desc">'+escHtml(w.description)+'</div>'+(creator?'<div class="t-micro c-text4 mt-1">By '+escHtml(creator.name)+'</div>':'')+'<div class="wheel-card-meta"><span class="wheel-meta-item">'+icon('users')+' '+fmt(w.memberCount)+'</span><span class="wheel-meta-item">'+escHtml(w.category)+'</span>'+(w.location?'<span class="wheel-meta-item">'+icon('map')+' '+escHtml(w.location)+'</span>':'')+(w.isEventWheel?'<span class="type-badge type-partnership" style="font-size:.6rem">Events</span>':'')+'</div></div><div class="wheel-card-footer"><span class="tier-badge tier-free">Open</span>'+(discover?'<button class="btn btn-teal btn-sm join-wheel-btn" data-wheel-id="'+w.id+'">Join</button>':'')+'</div></div>';
}
...
async function renderWheelDetail(){
  ...
  el.innerHTML=
    '<div class="page-head"><div class="flex gap-3 items-center">'+hexBadge(wheel,44)+'<div><h1 class="page-title" style="margin-bottom:0">'+escHtml(wheel.name)+'</h1><p class="page-sub">'+escHtml(wheel.description)+'</p>'+(creatorUser?'<p class="t-small c-text3 mt-1">Created by <a href="javascript:void(0)" onclick="navigate(\'profile\',{userId:\''+creatorUser.id+'\'})" style="color:var(--teal);font-weight:600;text-decoration:none">'+escHtml(creatorUser.name)+'</a></p>':'')+'</div></div><div class="page-actions">'+(isCreator?'<button class="btn btn-outline btn-sm" onclick="openInviteModal(\''+wheel.id+'\')">'+icon('users')+' Invite</button><button class="btn btn-outline btn-sm" onclick="openModal(\'modal-create-event\')">+ Event</button>':'')+(isCreator||isMember?'<button class="btn btn-outline btn-sm" onclick="shareWheel(\''+escHtml(wheel.slug||'')+'\',\''+escHtml(wheel.name).replace(/'/g,"\\\\'")+'\')">'+icon('link')+' Share Wheel</button>':'')+(isCreator?'<button class="btn btn-ghost btn-sm delete-wheel-btn" style="color:var(--red)" data-wheel-id="'+wheel.id+'" data-wheel-name="'+escHtml(wheel.name)+'">Delete Wheel</button>':(isMember?'<button class="btn btn-ghost btn-sm leave-wheel-btn" style="color:var(--red)" data-wheel-id="'+wheel.id+'" data-wheel-name="'+escHtml(wheel.name)+'">Leave Wheel</button>':''))+'<button class="btn btn-teal btn-sm" onclick="handlePostClick(\''+wheel.id+'\')">'+ icon('plus') +' Post</button></div></div>'+
