...
function renderProfile() {
  ...
  const picSection = isMe ? (
    ...
    [0,1,2,3,4].map(i => {
      ...
      if(pic) return '<div style="position:relative"><img src="'+pic+'" style="width:72px;height:72px;border-radius:var(--radius-sm);object-fit:cover;border:2px solid rgba(255,255,255,.2)"><label style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.4);border-radius:var(--radius-sm);opacity:0;cursor:pointer;transition:.2s" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0">'+icon('camera')+'<input type="file" accept="image/*" style="display:none" onchange="uploadPic(event,'+i+')"></label></div>';
      return '<label style="width:72px;height:72px;border-radius:var(--radius-sm);border:2px dashed rgba(255,255,255,.3);display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer;color:rgba(255,255,255,.5);font-size:.625rem;gap:.25rem">'+icon('camera')+'Photo '+(i+1)+'<input type="file" accept="image/*" style="display:none" onchange="uploadPic(event,'+i+')"></label>';
    }).join('') + '</div>'
  ) : ((u.profilePics||[]).filter(Boolean).length > 1 ?
    '<div style="display:flex;gap:.5rem;margin-top:1rem;flex-wrap:wrap">'+(u.profilePics||[]).slice(1).filter(Boolean).map(p=>'<img src="'+p+'" style="width:72px;height:72px;border-radius:var(--radius-sm);object-fit:cover">').join('')+'</div>' : '');
  ...
  '<div style="position:relative;display:inline-block;margin-bottom:1rem">'+(u.profilePics&&u.profilePics[0]?'<img src="'+u.profilePics[0]+'" style="width:80px;height:80px;border-radius:50%;object-fit:cover;border:3px solid rgba(255,255,255,.2)">':'<div class="profile-avatar-lg">'+initials(u.name)+'</div>')+
