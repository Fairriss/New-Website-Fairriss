...
// ── Auth ───────────────────────────────────────────────────────────────────
function renderAuth(){
  document.body.innerHTML='<div class="auth-screen"><div class="auth-brand" style="padding:0;overflow:hidden;position:relative"><img src="Fairriss_Art.png" alt="Fairriss" style="width:100%;height:100%;object-fit:cover;object-position:center"><div style="position:absolute;bottom:0;left:0;right:0;padding:2rem;background:linear-gradient(to top,rgba(15,31,61,.95),transparent)"><div class="auth-brand-logo" style="margin-bottom:.75rem"><svg width="34" height="34" viewBox="0 0 38 38" fill="none"><circle cx="19" cy="19" r="19" fill="currentColor"/><circle cx="19" cy="19" r="12" fill="#0F1F3D" opacity=".5"/><text x="19" y="24" text-anchor="middle" font-size="14" font-weight="900" fill="currentColor">F</text></svg><span class="auth-brand-logo-text">Fairriss</span></div><p class="auth-brand-tagline" style="font-size:1.0625rem;font-weight:600">Your Journey. Your Community. Your Future.</p></div><div class="auth-form-side"><h1>Welcome to Fairriss</h1><p class="auth-sub">Join the network where deals get done.</p><div class="auth-tabs"><div class="auth-tab active" data-tab="login">Sign In</div><div class="auth-tab" data-tab="signup">Create Account</div></div>'+
...
function renderShell(me){
  ...
  '<div class="mobile-nav-item '+(currentPage==='wheels'||currentPage==='wheel-detail'?'active':'')+'" onclick="navigate(\'wheels\')">'+
  ...
  '<div class="mobile-nav-item '+(currentPage==='opportunities'?'active':'')+'" onclick="navigate(\'opportunities\')">'+
  ...
  '<svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Me</div>'+
  '</nav>'+
...
async function renderWheels(){
  ...
  if(discoverWheels.length)html+='<h2 class="t-h2 mb-3 mt-4">Discover Wheels</h2><div class="wheel-grid">'+discoverWheels.map(w=>renderWheelCard(w,true)).join('')+'</div>';
  if(!myWheels.length&&!discoverWheels.length&&!popular.length)html+='<div class="empty-state"><div class="empty-icon">&#x2B22;</div><div class="empty-title">No Wheels yet</div><button class="btn btn-primary" onclick="openModal(\'modal-create-wheel\')">Create Your First Wheel</button></div>';
  el.innerHTML=html;
...
async function renderDealDetail(){
  ...
  '<div class="card card-sm" style="background:var(--surface)"><div class="t-label c-text4 mb-2">Fee Breakdown</div><div class="flex justify-between mb-1"><span class="t-small c-text3">Deal value</span><span class="t-small">'+fmtMoney(deal.priceCents/100)+'</span></div><div class="flex justify-between mb-1"><span class="t-small c-text3">Fairriss fee (10%)</span><span class="t-small c-red">-'+fmtMoney(fees)+'</span></div><div class="flex justify-between mb-2"><span class="t-small c-text3">Creator commission ('+deal.creatorCommissionPct+'%)</span><span class="t-small c-red">-'+fmtMoney(deal.priceCents/100*deal.creatorCommissionPct/100)+'</span></div><div class="divider" style="margin:.5rem 0"></div><div class="flex justify-between"><span class="t-body" style="font-weight:700">Seller receives</span><span class="t-body c-green" style="font-weight:700">'+fmtMoney(sellerGets)+'</span></div></div></div>'+
